...
Research pack (architecture, phases, provenance) lives in the Graycart Project store under `docs/graycart-gba/` until copied or linked into this repo’s `docs/` at scaffold. Family core API: `docs/graycart-family/` in that store (and sibling links from gba research doc `10-core-api-and-gb-reuse.md`).
...
Same posture as [graycart-gb `AGENTS.md`](https://github.com/graycart/graycart-gb/blob/main/AGENTS.md):
...
Default CI must stay green **without** BIOS images, commercial ROMs, or ignored ROM matrices. Never `cargo test -- --ignored` in the default workflow.
...
- In-repo: this file, `README.md`, `CONTRIBUTING.md`, and (when added) `docs/conformance.md` + BIOS obtain-your-own notes.
...
No emulator core implementation until the research review gate in `08-implementation-plan.md` §7 is accepted.
